...
class cp extends facade.Control
{
    ...
    async myProps(params) {
        ...
        let retProps = await this.core.service.gamegoldHelper.execute('prop.list', [0, user.domainId, null, cpInfo.result.cid]);
    ...
    async responseUser(params) {
        let rt = await this.core.service.gamegoldHelper.execute('token.user', [params.cp_name, params.uid, null, params.uid]);
        if(rt.code == 0) {
