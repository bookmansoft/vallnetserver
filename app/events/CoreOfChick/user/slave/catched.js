...
let {EntityType, ActivityType, em_Condition_Type, em_Condition_Checkmode, ReturnCode, NotifyType, UserStatus} = facade.const
let mails = facade.entities.mails
...
function handle(data) {
    ...
    msg.info.time = this.fileMap.DataConst.slave.catchTime;
    ...
    if(msg.info.src == data.user.openid){//抓捕者
        if(msg.info.code == ReturnCode.Success){
            ...
            if(msg.info.code == ReturnCode.Success){        
                ...
                this.notifyEvent('user.task', {use:data.user, data:{type:em_Condition_Type.totalSlaveCatch, value:1}});
                //累计分段积分
        ...
        if(msg.info.code == ReturnCode.Success){
            ...
            if(msg.info.code == ReturnCode.Success){
                //写报告
                this.GetMapping(EntityType.Mail).Create(data.user, msg, "system", data.user.openid);
