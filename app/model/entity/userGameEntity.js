...
let {UserGame} = require('../table/usergame');
let tableType = require('../../util/tabletype');

//用户微信账号(openid)
class userGameEntity extends BaseEntity
{
    //region 集合功能
    ...
    static get mapParams() {
        return {
            etype: tableType.userGame,                     //表类型
            model: UserGame,               //表映射类
            entity: userGameEntity,        //ORM映射类
        };
    }
    ...
    static async onCreate(item) {
        try{
            let it = await UserGame().create(item);
            await it.save();
    ...
    static onMapping(record){
        return new userGameEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await UserGame(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = userGameEntity;
