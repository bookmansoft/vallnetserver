...
class stockEntity extends BaseEntity
{
    ...
    static get mapParams() {
        return {
            etype: tableType.stock,                     //表类型
            model: Stock,               //表映射类
            entity: stockEntity,        //ORM映射类
        };
    }
    ...
    static async onCreate(item) {
        try{
            let it = await Stock().create(item);
            await it.save();
    ...
    static onMapping(record){
        return new stockEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await Stock(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = stockEntity;
