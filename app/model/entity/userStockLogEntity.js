...
class userStockLogEntity extends BaseEntity
{
    ...
    static async onCreate(item) {
        try{
            let it = await UserStockLog().create(item);
            await it.save();
    ...
    static onMapping(record){
        return new userStockLogEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await UserStockLog(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = userStockLogEntity;
