...
let {BlockGameProp} = require('../table/blockgameprop');
...
class blockGamePropEntity extends BaseEntity
{
    ...
    static async onCreate(item) {
        try{
            let it = await BlockGameProp().create(item);
            await it.save();
    ...
    static onMapping(record){
        return new blockGamePropEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await BlockGameProp(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
