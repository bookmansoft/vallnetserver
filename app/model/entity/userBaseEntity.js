...
let {UserBase} = require('../table/userbase')
let tableType = require('../../util/tabletype');

//基础用户信息
class userBaseEntity extends BaseEntity
{
    //region 集合功能
    ...
    static get mapParams() {
        return {
            etype: tableType.userBase,                     //表类型
            model: UserBase,               //表映射类
            entity: userBaseEntity,        //ORM映射类
        };
    }
    ...
    static async onCreate(item) {
        try{
            let it = await UserBase().create(item);
            await it.save();
            return it;
        }
        catch(e){
    ...
    static onMapping(record){
        return new userBaseEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await UserBase(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = userBaseEntity;
