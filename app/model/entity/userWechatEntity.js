...
let {UserWechat} = require('../table/userwechat');
let tableType = require('../../util/tabletype');

//用户微信账号(openid)
class userWechatEntity extends BaseEntity
{
    //region 集合功能
    ...
    static async onCreate(item) {
        try{
            let it = await UserWechat().create(item);
            await it.save();
    ...
    static onMapping(record){
        return new userWechatEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await UserWechat(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = userWechatEntity;
