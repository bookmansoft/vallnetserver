...
class VipDrawEntity extends BaseEntity
{
    ...
    static async onCreate(item) {
        try{
            let it = await VipDraw().create(item);
            await it.save();
    ...
    static onMapping(record){
        return new VipDrawEntity(record, facade.current);
    }
    ...
    static async onLoad(db, sa, pwd, callback){
        ...
        try {
            let ret = await VipDraw(db, sa, pwd).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = VipDrawEntity;
