...
class RedPackEntity extends BaseEntity
{
    ...
    static async onCreate(db, item) {
        try{
            let it = await redpack(db).create(item);
            await it.save();
    ...
    static onMapping(record, core) {
        return new RedPackEntity(record, core);
    }
    ...
    static async onLoad(db, callback){
        try {
            let ret = await redpack(db).findAll();
            ret.map(it=>{
                callback(it);
            });
        } catch(e) {
...
exports = module.exports = RedPackEntity;
