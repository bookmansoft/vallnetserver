...
class social extends facade.Control
{
        ...
        switch($msg.type){
            ...
            case NotifyType.slaveCommend: //奴隶：表扬
            {
                ...
                if(user.getActionMgr().Execute(this.core.const.ActionExecuteType.slaveCommend, 1, true)) {
                    ...
                    //成功后给奴隶主发放奖励
                    let bonus = [{type:ResType.Coin, num:500}];
                    user.getBonus(bonus);
            ...
            case NotifyType.slaveAvenge:
                {
                    ...
                    if($msg.info.code == ReturnCode.Success){
                        if(user.getActionMgr().Execute(this.core.const.ActionExecuteType.slaveAvenge, 1, true)) {
                            ...
                            //产生并发放300gold
                            let bonus = [{type:ResType.Coin, num:300}];
                            user.getBonus(bonus);
                            $msg.info.bonus = bonus;
                        }
                        else {
    ...
    async share(user, objData){
        ...
        if(!user.baseMgr.info.CheckStatus(UserStatus.isFirstShare)){
            ...
            user.getBonus({type:ResType.Coin, num:1000});
            user.getBonus({type:ResType.Item, id: 20, num: 1});
            ret.code = ReturnCode.Success;
            ret.data.bonus = [{type:ResType.Coin, num:1000},{type:ResType.Item, id:20, num:1}];
