...
class P502001 extends facade.Control
{
    ...
    queryItem($value, $data, user){
        let $hi = this.core.potentialConfig.getPetList()[$value.id];
        ...
        $data.items[$value.id].bp = bp.read(...this.core.ConfigMgr.ProfessionList()[$hi.profession].BP).multi((1+0.05*$value.getLevel())); //荣誉值集合 - 使用配置表进行初始化
    }
}
