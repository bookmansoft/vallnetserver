...
let {em_Ally_Oper, InviteType, ReturnCode} = facade.const
        ...
        switch(objData.oper){
            ...
            case em_Ally_Oper.inviteAllow: //只对加盟邀请有效
                if(objData.type == InviteType.AllyInvite){
                    $code = this.core.entities.AllyObject.InviteAccept(user, objData.id);
                }
                break;

            case em_Ally_Oper.inviteCancel:
                if(objData.type == InviteType.AllyInvite){
                    $code = this.core.entities.AllyObject.InviteCancel(user, objData.id);
                }
            ...
            case em_Ally_Oper.invite: //邀请加盟
                $code = this.core.entities.AllyObject.InviteSubmit(user, objData.id);
                break;
        }
