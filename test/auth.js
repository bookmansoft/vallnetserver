...
describe.only('钱包注册登录测试', () => {
    ...
    it('用户注册并登录 - 使用第三方授权模式', async () => {
        let ret = await remote.init().login({
            domain: 'CoreOfWallet',                           //认证模式
            openkey: `authwx.${Math.random()*1000000000 | 0}`, //中间证书，填写于 openkey 而非 openid 上，服务端转换最终的 openid 后下发给客户端
        });
        assert(ret, 'login failed');
    ...
    it('用户再次登录 - 后期绑定成功后，使用两阶段登录模式登录同一个账户', async () => {
        ...
        //当用户输入验证码时，使用事件驱动登录操作
        remote.events.emit('authcode', msg.data);
        //异步操作有一定延迟，如果此处时延设置不足，会导致后续流程因登录返回报文未到达、 openid 未正确设置而报校验错误
        await (async function(time){return new Promise(resolve =>{setTimeout(resolve, time);});})(3500);
